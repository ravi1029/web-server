      ...
      while (true) {
        ...
        try (Socket client = serverSocket.accept()) {
          ...
          // Decide how we'd like to respond
            ...
            if (resource.equals("/ws")) {
              System.out.println(resource.equals("/ws"));
              FileInputStream file = new FileInputStream("chat.js");
              ...
              clientOutput.flush();
            } else if (resource.equals("/wel")) {
              OutputStream clientOutput = client.getOutputStream();
              clientOutput.write(("HTTP/1.1 200 OK\r\n").getBytes());
              clientOutput.write(("\r\n").getBytes());
              clientOutput.write(("WELCOME").getBytes());
              clientOutput.flush();
            } else {
              OutputStream clientOutput = client.getOutputStream();
              clientOutput.write(("HTTP/1.1 200 OK\r\n").getBytes());
              clientOutput.write(("\r\n").getBytes());
              clientOutput.write(("Welcome to our web server !!").getBytes());
            //  clientOutput.flush();
            }
            // Send back the appropriate thing based on resource

            // Just send back a simple "WELCOME"
